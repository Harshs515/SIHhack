...
import { Shield, LayoutDashboard, MapPin, Cpu, GitFork, ShieldAlert, BellRing, FileSpreadsheet, BarChart3, Network } from 'lucide-react';
...
export default function Navbar({ isRunningML, onTriggerML, activeAlertsCount = 2 }) {
  ...
  return (
    <header style={{
      ...
      background: 'linear-gradient(to bottom, rgba(7,11,20,0.97) 0%, rgba(7,11,20,0.85) 75%, rgba(7,11,20,0) 100%)',
      ...
      <div style={{ height: '56px', display: 'flex', alignItems: 'center', gap: '0' }}>

        {/* Logo */}
        <Link to="/dashboard" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0, marginRight: '20px' }}>
          <div style={{
            width: '32px', height: '32px', borderRadius: '8px',
            background: 'linear-gradient(135deg, #00e5ff 0%, #3a7bd5 100%)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: '#040914', boxShadow: '0 0 14px rgba(0,229,255,0.4)', flexShrink: 0,
          }}>
        ...
        {/* Divider */}
        <div style={{ width: '1px', height: '20px', background: 'rgba(255,255,255,0.08)', marginRight: '20px', flexShrink: 0 }} />
        ...
        {/* Right: live clock only */}
        <div style={{ marginLeft: '16px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '5px' }}>
          <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#00e676', boxShadow: '0 0 6px #00e676', display: 'inline-block' }} />
          <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', letterSpacing: '0.01em' }}>{time}</span>
        </div>
      </div>
